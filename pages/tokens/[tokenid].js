...
function Token({ asset }) {
  ...
  const nfturl = `https://gateway.ipfs.io/ipfs/${asset?.tokens[0].metaDataUri?.substr(7,50)}`;
  ...
  const metadata = async () => {
    const { data } = await axios.get(
      `https://gateway.ipfs.io/ipfs/${asset?.tokens[0].metaDataUri?.substr(7, 50)}`
    );
    setResponse(data);
    setImage(data.image);
  ...
  useEffect(() => {
    metadata();
  }, [asset?.tokens[0].metaDataUri?.substr(7, 50)]);
  let preuri = image.substr(7, 50);
      ...
      <div className="max-w-[1400px] mx-auto bg-[#f8f7fc] p-8 dark:bg-[#131417] my-8 rounded-3xl">
        <div className="flex flex-col lg:flex-row gap-x-8">
          <div className="w-full lg:w-[50%]" onClick={() => isSetFull(true)}>
            <AssetComp
              uri={
                asset? asset.tokens[0].metaDataUri?.substr(7, 50)
                  : ""
              }
            />
          </div>
              ...
              <div className="">
                <div className="">
                  <div className="rounded-3xl w-full px-4 py-3 bg-white dark:bg-[#1e1f26] myshadow text-[#253262]">
                    ...
                    <button
                      onClick={() =>
                        buyNFT(asset.tokens[0], setmodel, setmodelmsg)
                      }
